...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Human–LLM vignettes occupy selected connecting passages only.
  // Original scene coordinates and sizes are unchanged.
  var gapScenes = [
    ['chat', null, 'translation', null, 'voice'],
    ['images', 'story', null, 'agents']
  ];
  function addGapScene(svg, connector, width, from, to, side, index) {
    var scene = gapScenes[side][index];
    if (!scene || to - from < 160) return;
    var size = Math.min(126, width * .7, to - from - 40);
    var onRight = (index + side) % 2 === 1;
    var x = onRight ? width - size : 0;
    var y = from + (to - from - size) / 2;
    // Let the thread disappear behind the vignette rather than cross its face.
    var maskId = 'gap-scene-clearance-'+side+'-'+index;
    var mask = element('mask', {id:maskId, maskUnits:'userSpaceOnUse', x:0, y:from, width:width, height:to-from});
    mask.appendChild(element('rect', {x:0, y:from, width:width, height:to-from, fill:'white'}));
    mask.appendChild(element('rect', {x:x, y:y+size*.07, width:size, height:size*.86, fill:'black'}));
    ...
    blocks.forEach(function (block, index) {
      ...
      if (index < blocks.length - 1) {
        ...
        if (to > from) {
          ...
          addGapScene(svg, connector, width, from, to, side, index);
        }
        y += gap;
      }
    });
